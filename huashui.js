  ...
  initComponent() {
    Vue.component("router-link", {
      ...
      render(h) {
        ...
        return h(
          "a",
          { to: this.$props.to, "@click": this.onClick },
          "<slot></slot>"
        );
      },
      methods: {
...
const arrayProtoObj = Object.create(arrayProto);
const methodsToPatch = ["push", "pop"];

const defineReacitve = function (target, key) {
  //
};

methodsToPatch.forEach((method) => {
  const origin = arrayProto[method];
  Object.defineProperty(arrayProtoObj, method, {
    configurable: false,
    enumerable: false,
    writable: true,
    value(...args) {
